    ...
    private = list(
      ...
      .init = function() {
        ...
        if (!private$.ready$ready) {
          if (private$.ready$report) {
            self$results$info$addRow(
              "info",
              list(info = "Setup", specs = private$.ready$reason)
            )
          }
          return()
        }
        ...
        aSmartObj <- SmartTable$new(
          self$results$fit$modelbaseline,
          runner_machine
        )
        aSmartObj$activateOnData <- TRUE
        ...
        aSmartObj <- SmartTable$new(
          self$results$models$coefficients,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        ...
        aSmartObj <- SmartTable$new(
          self$results$models$composites,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$ci(NULL, self$options$ci_width)
        ...
        aSmartObj <- SmartTable$new(
          self$results$models$covariances,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$ci(NULL, self$options$ci_width)
        ...
        aSmartObj <- SmartTable$new(
          self$results$models$intercepts,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$ci(NULL, self$options$ci_width)
        ...
        aSmartObj <- SmartTable$new(
          self$results$models$thresholds,
          runner_machine
        )
        ...
        aSmartObj <- SmartTable$new(
          self$results$additional$reliability,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$activated <- (self$options$reliability &
          is.something(runner_machine$latent))
        ...
        aSmartObj <- SmartTable$new(
          self$results$additional$mardia,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        ladd(private$.smartObjs) <- aSmartObj
        ...
        aSmartObj <- SmartTable$new(
          self$results$covariances$observed,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$expandOnInit <- TRUE
        ...
        aSmartObj <- SmartTable$new(
          self$results$covariances$implied,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$expandOnInit <- TRUE
        ...
        aSmartObj <- SmartTable$new(
          self$results$covariances$residual,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$expandOnInit <- TRUE
        ...
        aSmartObj <- SmartTable$new(
          self$results$covariances$combined,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$expandOnInit <- TRUE
        ...
        aSmartObj <- SmartTable$new(
          self$results$covariances$latent,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        aSmartObj$activated <- ((self$options$cov.lv) &
          is.something(runner_machine$latent))
        aSmartObj$expandOnInit <- TRUE
        aSmartObj$expandFrom <- 3
        ladd(private$.smartObjs) <- aSmartObj
        ...
        aSmartObj <- SmartTable$new(
          self$results$modification$indices,
          runner_machine
        )
        aSmartObj$spaceBy <- "lgroup"
        ladd(private$.smartObjs) <- aSmartObj
        ...
        aSmartObj <- SmartTable$new(
          self$results$meas.invariance$meas.invariance,
          runner_machine
        )
        ...
        ######## plotting class #######
        private$.plotter_machine <- Plotter$new(
          self,
          runner_machine,
          self$results$pathgroup
        )
      ...
      .run = function() {
        ...
        if (self$options$outputMeasInv) {
          ...
          self$results$meas.invariance$meas.invariance <- inv_result
        }
        ...
        if (is.something(private$.plotter_machine$warnings$diagram)) {
          for (i in seq_along(private$.plotter_machine$warnings$diagram)) {
            self$results$pathgroup$notes$addRow(
              i,
              list(message = private$.plotter_machine$warnings$diagram[[i]])
            )
          }
      ...
      .showDiagram = function(image, ggtheme, theme, ...) {
        ...
        if (!isFALSE(res$error)) {
          ...
          if (
            length(grep("Circle layout only supported", res$error, fixed = T)) >
              0
          ) {
        ...
        if (!isFALSE(res$error)) {
          private$.plotter_machine$dispatcher$warnings <- list(
            topic = "pathgroup_notes",
            message = res$error
          )
        }
